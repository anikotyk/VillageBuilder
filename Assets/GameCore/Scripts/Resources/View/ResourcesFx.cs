...
public class ResourcesFx : MonoBehaviour
{
    ...
    public int Visualize(IEnumerable<StackItem> stackItems, Vector3 destination, Action onReceiveDestination = null)
    {
        var resourceView = _resourcePresenter.ResourceViews.Find(x => x.ItemType == stackItems.First().Type).IconImage;
        return Visualize(stackItems, destination, _fxParent, resourceView.transform.position, onReceiveDestination);
    }
    ...
    {
        _player.Stack.TryToGetStack(ItemType.Wood, out IStack stack);
