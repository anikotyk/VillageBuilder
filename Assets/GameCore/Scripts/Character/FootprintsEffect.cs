...
public class FootprintsEffect : MonoBehaviour
{
    ...
    private List<Footprint> Footprints
    {
        get
        {
            if (_footprints.Count == 0)
            {
                ...
                {
                    _footprints.Add(new Footprint(_footprint, _startScale, _startRotation, _color, _timer));
                }
            }
    ...
    private class Footprint
    {
        ...
        private TimerDelay _lifeTimer;

        private Tweener _zoomTweener;
        
        public Footprint(Sprite footprint, Vector3 startScale, Vector3 startRotation, Color color, Timer timer)
        {
        ...
        private void Hide(float zoomTime)
        {
            _zoomTweener?.Kill();
            _lifeTimer?.Kill();
            _zoomTweener = _spriteRenderer.transform.DOScale(Vector3.zero, zoomTime).OnComplete(()=> _spriteRenderer.gameObject.SetActive(false));
        }
        ...
        public void Show(Transform followTransform, float lifeTime, float zoomTime)
        {
            ...
            _zoomTweener = transform.DOScale(_startScale, zoomTime).SetEase(Ease.OutBack);
            
            _lifeTimer = _timer.ExecuteWithDelay(() => Hide(zoomTime), lifeTime);
        }
    }
}
